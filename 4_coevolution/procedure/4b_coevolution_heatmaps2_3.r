...
allfams <- sort(unique(c(tissue.fams,skeleton.fams,mucus.fams)))
...
sample_data(otu_data_temp)$has_skeleton <- 'no'
sample_data(otu_data_temp)$has_skeleton[sample_data(otu_data_temp)$taxonomy_string_to_order=='Cnidaria_Anthozoa_Helioporaceae'] <- 'yes'
sample_data(otu_data_temp)$has_skeleton[sample_data(otu_data_temp)$taxonomy_string_to_order=='Cnidaria_Hydrozoa_Anthoathecata'] <- 'yes'
sample_data(otu_data_temp)$has_skeleton[sample_data(otu_data_temp)$taxonomy_string_to_order=='Cnidaria_Anthozoa_Scleractinia'] <- 'yes'
...
for(compart in c('T','S','M')) {
    ...
    if (compart == 'S') {
        comp.pruned <- subset_samples(otu_data_temp, tissue_compartment=='S' & has_skeleton=='yes')
    } else {
        comp.pruned <- subset_samples(otu_data_temp, tissue_compartment==compart)
    }

	for(taxon in famlist[[compart]]) {
    
        tre <- read.nexus(Sys.glob(paste0('/Users/Ryan/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_coevolutio*/',compartments[[compart]],'/',taxon,'/beast/',taxon,'_final_tree.tree')))


        otu_data <- merge_phyloseq(comp.pruned,map,tre)
        ...
        ref_tips <- tre$tip.label[!tre$tip.label %in% taxa_names(comp.pruned)]
        ...
        outer_data <- merge_phyloseq(comp.pruned,ref_object,map,tre)
        ...
        for(type in c('with_reference','without_reference')) {
            ...
            dir.create(paste0('/Volumes/Moorea/4-coevolution_binary_presence/',compartments[[compart]],'/',taxon,'/'), recursive=T)

            pdf(paste0('/Volumes/Moorea/4-coevolution_binary_presence/',compartments[[compart]],'/',taxon,'/',taxon,'_',type,'_heatmap_scalebyrow_finer.pdf'))
            ...
            pdf(paste0('/Volumes/Moorea/4-coevolution_binary_presence/',compartments[[compart]],'/',taxon,'/',taxon,'_',type,'_heatmap_scalebycol_finer.pdf'))
            ...
            pdf(paste0('/Volumes/Moorea/4-coevolution_binary_presence/',compartments[[compart]],'/',taxon,'/',taxon,'_',type,'_heatmap_noscale_finer.pdf'))
            ...
            pdf(paste0('/Volumes/Moorea/4-coevolution_binary_presence/',compartments[[compart]],'/',taxon,'/',taxon,'_',type,'_heatmap_sqrt_scalebyrow_finer.pdf'))
            ...
            pdf(paste0('/Volumes/Moorea/4-coevolution_binary_presence/',compartments[[compart]],'/',taxon,'/',taxon,'_',type,'_heatmap_scalebytaxamaxrow_finer.pdf'))
            ...
            pdf(paste0('/Volumes/Moorea/4-coevolution_binary_presence/',compartments[[compart]],'/',taxon,'/',taxon,'_',type,'_heatmap_scalebytaxamax_finer.pdf'))
            maxrel <- t(apply(baserel_filt, 1, function(x) x/max(x)))
            ...
            pdf(paste0('/Volumes/Moorea/4-coevolution_binary_presence/',compartments[[compart]],'/',taxon,'/',taxon,'_',type,'_heatmap_sqrt_scalebytaxamax_finer.pdf'))
            sqrtrel <- sqrt(baserel_filt)
