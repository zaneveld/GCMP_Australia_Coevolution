...
datframe2 <- droplevels(datframe[!is.na(datframe$tissue_compartment) & (datframe$tissue_compartment == 'T'), ])
datframe2 <- droplevels(datframe[!is.na(datframe$tissue_compartment) & (datframe$tissue_compartment == 'S'), ])
datframe2 <- droplevels(datframe[!is.na(datframe$tissue_compartment) & (datframe$tissue_compartment == 'M'), ])
...
names(hostvect) <- datframe2$X.SampleID
...
pruned.hosttree.samples.dichotomous <- ladderize(multi2di(pruned.hosttree.samples))
pruned.hosttree.samples.dichotomous$edge.length <- pruned.hosttree.samples.dichotomous$edge.length + 0.0001


datframe2$X.SampleID[!datframe2$X.SampleID %in% pruned.hosttree.samples.dichotomous$tip.label] <- NA



#dat_species_cat <- aggregate(datframe2[,c('geographical_area')], by=list(X16S_tree_name=datframe2$X16S_tree_name), FUN=function(x) names(which.max(table(datframe2[,x]))))
#rownames(dat_species_cat) <- dat_species_cat$X16S_tree_name


dat_species <- aggregate(datframe2[,c('equitability','PD_whole_tree','observed_otus')], by=list(X16S_tree_name=datframe2$X16S_tree_name), FUN='mean')
rownames(dat_species) <- dat_species$X16S_tree_name



dat_species <- dat_species[,-1]
multiPhylosignal(dat_species,pruned.hosttree)



rownames(datframe2) <- datframe2$X.SampleID
dat2 <- datframe2[,c('equitability','PD_whole_tree','observed_otus')]

multiPhylosignal(dat2,pruned.hosttree.samples.dichotomous)
...
dat3 <- dat_species
dat3$X16S_tree_name.phy <- rownames(dat3)



Gs <- list(V=diag(3), nu=0.02)

prioriw <- list(R=list(V=diag(3),nu=0.02), G=list(G1=Gs))


mc <- MCMCglmm(cbind(PD_whole_tree,equitability,observed_otus) ~ 0 + trait,
random = ~ idh(trait):X16S_tree_name.phy,
family=c('gaussian','gaussian','gaussian'),
data=dat3,
nitt=1300000,
thin=100,
