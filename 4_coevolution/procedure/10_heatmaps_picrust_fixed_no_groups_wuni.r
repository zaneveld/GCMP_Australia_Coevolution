...
#,'S'
for(compart in c('T','M')) {
    
    sig.host <- list()
    sig.hostphy <- list()
    sig.hosts <- list()

    host.otu <- read.table(paste0('~/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_picrust_raw/',compart,'_parsed/',compart,'_Host.otu.txt'), sep='\t', header=T, as.is=T)
    host.otu.hostphy <- read.table(paste0('~/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_picrust_raw/',compart,'_parsed/',compart,'_Host.otu.hostphy.txt'), sep='\t', header=T, as.is=T)
    ...
    for(direction in c('positive','negative')) {
        ...
        write(sig.host[[direction]],file=paste0('~/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_picrust_raw/',compart,'_parsed/',compart,'_',direction,'_sigs_by_host.txt'),sep='\n')
        ...
        write(sig.hostphy[[direction]],file=paste0('~/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_picrust_raw/',compart,'_parsed/',compart,'_',direction,'_sigs_by_hostphy.txt'),sep='\n')
    ...
    for(set in names(sets)) {
        ...
        pdf(paste0('~/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_picrust_raw/',compart,'_parsed/wuni_',compart,'_',set,'_heatmap_scalebytaxamax.pdf'))
        ...
        pdf(paste0('~/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_picrust_raw/',compart,'_parsed/wuni_',compart,'_',set,'_heatmap_sqrt_scalebytaxamax.pdf'))
        ...
        pdf(paste0('~/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_picrust_raw/',compart,'_parsed/wuni_',compart,'_',set,'_heatmap_sqrt.pdf'))
        sqrtrel <- sqrt(baserel_filt)
        heatmap(sqrtrel, Rowv=as.dendrogram(hclbacttree), Colv=as.dendrogram(hclhosttree), col=brewer.pal(9,"Blues"), cexCol=0.2, cexRow=0.1, scale='none')
        dev.off()
