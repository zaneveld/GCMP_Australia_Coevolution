...
tissue.fams <-'f__Endozoicimonaceae'
...
for(compart in c('T','S','M')) {
    ...
    for(taxon in famlist[[compart]]) {
		...
		host.otuAx <- host.otuA
        host.otuAx[as.matrix(kronecker(Diagonal(nrow(inv.host)),inv.bact)) != 0 | as.matrix(kronecker(inv.host,Diagonal(nrow(inv.bact)))) != 0] <- 0
        diag(host.otuAx) <- diag(host.otuA)


        rownames(host.otuA)<-apply(expand.grid(rownames(inv.bact), rownames(inv.host)), 1, function(x){paste(x[2],x[1], sep=".")})
        rownames(host.otuAx)<-rownames(host.otuAS)<-rownames(host.otuSA)<-rownames(host.otuA)
        ...
        assocs$Host.otu.cophyX<-paste(assocs$X16S_tree_name, assocs$otu, sep=".")  # phylogentic coevolution effect ONLY
        assocs$Host.otu.cophyX[is.na(assocs$X16S_tree_name)] <- NA
        ...
        randfacts <- c('otu.phy','otu','geo.otu','Host.otu.hostphy','Host.otu.otuphy','Host.otu','Host.otu.cophy','Host.otu.cophyX')
...
family="poisson",
data=assocs,
ginverse=list(otu.phy=inv.bact, Host.otu.hostphy=host.otuAS, Host.otu.otuphy=host.otuSA, Host.otu.cophy=host.otuA, Host.otu.cophyX=host.otuAx),
prior=priorC,
nitt=125000,
thin=5,
burnin=25000,
