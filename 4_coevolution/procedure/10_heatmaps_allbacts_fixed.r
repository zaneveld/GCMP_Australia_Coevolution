...
for(compart in c('T','M','S')) {
    ...
    host.otu <- read.table(paste0('~/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_allbacts_fixed_backup/',compart,'_parsed/',compart,'_Host.otu.txt'), sep='\t', header=T, as.is=T)
    ...
    func.otu <- read.table(paste0('~/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_allbacts_fixed/',compart,'_parsed/',compart,'_func.otu.txt'), sep='\t', header=T, as.is=T)
    ...
    for(direction in c('positive','negative')) {
        ...
        func.otu.sig <- direct.function(func.otu)
        func.otu.parsed <- do.call('rbind',strsplit(func.otu.sig[,1],'.', fixed=T))
        
        
        sig.func[[direction]] <- unique(func.otu.parsed[,2])
        write(sig.func[[direction]],file=paste0('~/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_allbacts_fixed/',compart,'_parsed/',compart,'_',direction,'_sigs_by_func.txt'),sep='\n')
        
        hosts.funcs[[direction]] <- unique(c(sig.hosts[[direction]],sig.func[[direction]]))
        
    }
    
    sets <- list(hosts=unlist(sig.hosts, use.names=F),functions=unlist(sig.func, use.names=F), hosts_and_functions=unlist(hosts.funcs, use.names=F))
