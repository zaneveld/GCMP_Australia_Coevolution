...
map[map=='Unknown'] <- NA
biom_object <- import_biom('/Users/Ryan/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_coevolution/skeleton/skeleton_MED_table.biom')
...
newer <- phyloseq(otu_table(otu_data),sample_data(otu_data))
good_tax <- import_biom('/Users/Ryan/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_subsets/chromerida_S/MED_chromerida_newtax.json')

otu_data <- merge_phyloseq(newer,tax_table(good_tax))

rm(list=c('biom_object','otu_data_full','otu_data_pruned','newer','good_tax'))
gc()

sample_data(otu_data)$has_skeleton <- 'no'
sample_data(otu_data)$has_skeleton[sample_data(otu_data)$taxonomy_string_to_order=='Cnidaria_Anthozoa_Helioporaceae'] <- 'yes'
sample_data(otu_data)$has_skeleton[sample_data(otu_data)$taxonomy_string_to_order=='Cnidaria_Hydrozoa_Anthoathecata'] <- 'yes'
sample_data(otu_data)$has_skeleton[sample_data(otu_data)$taxonomy_string_to_order=='Cnidaria_Anthozoa_Scleractinia'] <- 'yes'
...
for(compart in c('S')) {
    ...
    comp.pruned <- subset_samples(otu_data, tissue_compartment=='S' & has_skeleton=='yes')
    
    for(taxon in famlist[[compart]]) {
        
        dir.create(paste0('/Users/Ryan/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_subsets/chromerida_S/',compartments[[compart]],'/',taxon,'_binary_presence/'), recursive=T)
        
        tre <- read.nexus(paste0('/Users/Ryan/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_subsets/chromerida_S/f__Chromerida/beast/',taxon,'_final_tree.tree'))
        ...
        write.table(pedigree_hosts,file=paste0('/Users/Ryan/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_subsets/chromerida_S/',compartments[[compart]],'/',taxon,'_binary_presence/',taxon,'_host_pedigree.txt'),sep='\t',quote=F,row.names=F)
        ...
        write.table(pedigree_bacts,file=paste0('/Users/Ryan/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_subsets/chromerida_S/',compartments[[compart]],'/',taxon,'_binary_presence/',taxon,'_bact_pedigree.txt'),sep='\t',quote=F,row.names=F)
        ...
        save.image(file=paste0('/Users/Ryan/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_subsets/chromerida_S/',compartments[[compart]],'/',taxon,'_binary_presence/',taxon,'_mcmc_setup.RData'))
    }
}