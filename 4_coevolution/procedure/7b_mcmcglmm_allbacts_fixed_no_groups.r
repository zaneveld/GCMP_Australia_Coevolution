...
for(tiscomp in c('M','T','S')) {
    ...
    assocs <- merge(assocs,sample_data(pruned)[,c('X16S_tree_name','geographic_area','sample_sum','colony_name','functional_group_sensu_darling','disease_category')],by.x='sample',by.y=0,all=F)
