...
datpos <- read.table('/Users/Ryan/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_allbacts_mcmc_otu/positive_associations.txt', sep='\t', header=T, as.is=T)

datneg <- read.table('/Users/Ryan/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_allbacts_mcmc_otu/positive_associations.txt', sep='\t', header=T, as.is=T)

dat <- rbind(datpos,datneg)

for(compart in c('T','M','S')) {
    
    for(factor in c('X16S_tree_name','geographic_area','X16S_tree_name.phy')) {
    
        sigs <- unique(datpos$otu[dat$compartment==compart & dat$factor==factor])
        ...
        pdf(paste0('~/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_allbacts_mcmc_otu/wuni_',compart,'_',factor,'_heatmap_scalebytaxamax.pdf'))
        ...
        pdf(paste0('~/Dropbox/Selectively_Shared_Vega_Lab_Stuff/GCMP/Projects/Australia_Coevolution_Paper/16S_analysis/4_coevolution/output/mcmcglmm_allbacts_mcmc_otu/wuni_',compart,'_',factor,'_heatmap_sqrt_scalebytaxamax.pdf'))
